...
namespace vision {
    namespace {
        ...
        // An output tensor is at minimum [detection_count, field_count]; a batch
        // dimension may precede those two.
        constexpr int kMinOutputDims = 2;
        ...
        // TensorRT's own diagnostics land in the same sink as ours; the log level
        // decides what survives instead of the hard kWARNING cut this used to have.
    ...
    TensorRtEngine::TensorRtEngine(const std::string& engine_path) {
        ...
        LOG_INFO("Engine") << device_buffers_.size() << " tensors | output ["
    ...
    void TensorRtEngine::BindTensor(int tensor_index) {
        ...
        // Buffers are allocated once and never move, so binding here keeps Infer()
        // down to the enqueue itself.
